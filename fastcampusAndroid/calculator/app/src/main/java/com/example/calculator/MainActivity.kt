...
class MainActivity : AppCompatActivity() {
    ...
    fun equalClicked(view: View) {
        if (firstNumberText.isEmpty() || secondNumberText.isEmpty() || operatorText.isEmpty()) {
            Toast.makeText(this, "수식이 잘못되었어요.", Toast.LENGTH_SHORT).show()
            return
        }
    ...
    private fun updateEquationTextView() {
        ...
        binding.equationTextView.text = getString(
            R.string.updateEquationTextView,
            firstFormattedNumber,
            operatorText,
            secondFormattedNumber
        )
